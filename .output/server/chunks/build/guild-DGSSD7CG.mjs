...
import { l as useAsyncData } from './server.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { data } = ([__temp, __restore] = withAsyncContext(async () => useAsyncData("mountains", async () => {
      const res = await $fetch("https://api.s22y.moe/lodestar/api/guild");
    ...
    return (_ctx, _push, _parent, _attrs) => {
      if (unref(data)) {
        ...
        ssrRenderList(unref(sorting), (item, index) => {
          _push(`<div><div class="bg-default border-default flex flex-col gap-1 rounded-lg border-1 px-3 py-2"><div class="flex items-center gap-2"><img${ssrRenderAttr("src", `https://mc-heads.net/head/${item.creator}`)} alt="item.creator" class="mb-0! w-10!"><div class="flex flex-col justify-center"><span>${("toColoredHtml" in _ctx ? _ctx.toColoredHtml : unref(toColoredHtml))(item.guild_name) ?? ""}</span><span class="text-sm text-gray-600"> 会长: ${ssrInterpolate(item.creator)}</span></div></div><p class="text-sm text-gray-600">${ssrInterpolate(item.description)}</p><div class="flex w-full"><div class="flex w-1/5 flex-col gap-0.5"><span class="text-xs text-gray-400">评分</span><span class="${ssrRenderClass({ "text-teal-500": index === 2, "text-indigo-500": index === 1, "text-rose-500": index === 0 })}">${ssrInterpolate(item.score)}</span></div><div class="flex w-1/5 flex-col gap-0.5"><span class="text-xs text-gray-400">等级</span><span>${ssrInterpolate(item.level)}</span></div><div class="flex w-1/5 flex-col gap-0.5"><span class="text-xs text-gray-400">会员</span><span>${ssrInterpolate(item.member_count)}</span></div><div class="flex w-1/5 flex-col gap-0.5"><span class="text-xs text-gray-400">活跃</span><span>${ssrInterpolate(item.prosperity_degree)}</span></div><div class="flex w-1/5 flex-col gap-0.5"><span class="text-xs text-gray-400">资金</span><span>${ssrInterpolate(item.money)}</span></div></div></div></div>`);
        });
        _push(`<!--]--></div></div>`);
      } else {
        _push(`<!---->`);
...
//# sourceMappingURL=guild-DGSSD7CG.mjs.map
